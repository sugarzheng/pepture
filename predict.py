...
predict_pdb = open("results/ABPP/input.dat", "r").readlines()
prediction=[]
os.system("rm results/ABPP/predict_num -rf")
batch_size = 2048
peptide_num=16
img_rows, img_cols = 20, peptide_num+1
#model = load_model('models/binary_20170504.h5')
model = load_model('models/ABPP_active.h5')
count=0
for (i,line) in enumerate(predict_pdb):
	...
	if i%batch_size!=0 and i!=(len(predict_pdb)-1) :	
		...
		output=open("results/ABPP/predict_num",'a')
