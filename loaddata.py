...
#			if line not in (train + test + validation):
			if i%3 != 0:
				...
				if i%6!=0:
					...
					validation.append(line.strip()+" "+str(label))
	return (train,test,validation)
def reverse_add(non_active_site):
	reverse_list=list()
...
def length_classification(file1,file2,num):
	...
	random.shuffle(active_site)
	random.shuffle(non_active_site)
	(train,test,validation)=data_append_num(non_active_site,(train,test,validation),num,0)
	(train,test,validation)=data_append_num(reverse_add(non_active_site),(train,test,validation),num,0)
